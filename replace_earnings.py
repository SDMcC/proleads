...
# Find and replace the EarningsTab function
old_function = '''function EarningsTab() {
  return (
...
new_function = '''function EarningsTab() {
...
if old_function in content:
    ...
    print('Old function not found exactly')
    # Let's try to find what's actually there
    import re
    match = re.search(r'function EarningsTab\(\).*?^}', content, re.MULTILINE | re.DOTALL)
    if match:
        print('Found EarningsTab function:')
        print(repr(match.group(0)))
    else:
        print('EarningsTab function not found at all')
