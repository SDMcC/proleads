...
const redirectUrl = `https://marketing-hub-162.preview.emergentagent.com/import?user_id=${userId}&file_id=${file.file_id}&source=proleads`;
...
**Resulting URL:**
```
https://marketing-hub-162.preview.emergentagent.com/import?user_id=user-123&file_id=csv-456&source=proleads&sso_token=eyJ...
```
...
```javascript
const response = await fetch('https://marketing-hub-162.preview.emergentagent.com/api/sso/verify', {
  method: 'POST',
  headers: {
...
```javascript
const csvResponse = await fetch('https://marketing-hub-162.preview.emergentagent.com/api/integrations/csv-export', {
  method: 'POST',
  headers: {
    'X-API-Key': 'sendloop_live_key_yalmQ6YGLT2PYPTAfvkHMuHbqJkw98Z1NFFYeWleuC8',
    'Content-Type': 'application/json'
...
curl -X POST "https://marketing-hub-162.preview.emergentagent.com/api/sso/initiate" \
  ...
  -d '{
    "target_app": "sendloop",
    "redirect_url": "https://marketing-hub-162.preview.emergentagent.com/import?user_id=user-123&file_id=csv-456&source=proleads"
  }'
...
curl -X POST "https://marketing-hub-162.preview.emergentagent.com/api/integrations/csv-export" \
  -H "X-API-Key: sendloop_live_key_yalmQ6YGLT2PYPTAfvkHMuHbqJkw98Z1NFFYeWleuC8" \
  -H "Content-Type: application/json" \
  -d '{
    "user_id": "user-123",
