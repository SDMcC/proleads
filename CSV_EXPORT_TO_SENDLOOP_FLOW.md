...
const redirectUrl = `https://payment-flow-70.preview.emergentagent.com/import?user_id=${userId}&file_id=${file.file_id}&source=proleads`;
...
**Resulting URL:**
```
https://payment-flow-70.preview.emergentagent.com/import?user_id=user-123&file_id=csv-456&source=proleads&sso_token=eyJ...
```
...
```javascript
const response = await fetch('https://payment-flow-70.preview.emergentagent.com/api/sso/verify', {
  method: 'POST',
  headers: {
...
```javascript
const csvResponse = await fetch('https://payment-flow-70.preview.emergentagent.com/api/integrations/csv-export', {
  method: 'POST',
  headers: {
    'X-API-Key': 'sendloop_live_key_yalmQ6YGLT2PYPTAfvkHMuHbqJkw98Z1NFFYeWleuC8',
    'Content-Type': 'application/json'
...
curl -X POST "https://payment-flow-70.preview.emergentagent.com/api/sso/initiate" \
  ...
  -d '{
    "target_app": "sendloop",
    "redirect_url": "https://payment-flow-70.preview.emergentagent.com/import?user_id=user-123&file_id=csv-456&source=proleads"
  }'
