...
class ProleadsIntegration:
    def __init__(self):
        ...
        # ✅ NEW
        self.api_url = os.environ.get("PROLEADS_API_URL", "https://payment-flow-70.preview.emergentagent.com")
...
USER_TOKEN=$(curl -s "https://payment-flow-70.preview.emergentagent.com/api/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"email":"testuser@example.com","password":"password"}' | grep -o '"token":"[^"]*"' | cut -d'"' -f4)

# Generate SSO token
curl -X POST "https://payment-flow-70.preview.emergentagent.com/api/sso/initiate" \
  -H "Authorization: Bearer $USER_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "target_app": "sendloop",
    "redirect_url": "https://payment-flow-70.preview.emergentagent.com/dashboard"
  }' | python3 -m json.tool
```
...
```json
{
  ...
  "redirect_url": "https://payment-flow-70.preview.emergentagent.com/dashboard?sso_token=eyJ..."
}
```
...
| Endpoint | Method | URL |
|----------|--------|-----|
| SSO Initiate | POST | `https://payment-flow-70.preview.emergentagent.com/api/sso/initiate` |
| SSO Verify | POST | `https://payment-flow-70.preview.emergentagent.com/api/sso/verify` |
| SSO User Info | GET | `https://payment-flow-70.preview.emergentagent.com/api/sso/user-info` |
| CSV Export | POST | `https://payment-flow-70.preview.emergentagent.com/api/integrations/csv-export` |
| CSV List | GET | `https://payment-flow-70.preview.emergentagent.com/api/integrations/csv-files` |
...
### For Preview/Testing:
- Use: `https://payment-flow-70.preview.emergentagent.com`
...
- [ ] Update `PROLEADS_API_URL` to `https://payment-flow-70.preview.emergentagent.com`
