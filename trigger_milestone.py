...
def trigger_milestone():
    ...
    try:
        ...
        admin_response = requests.post('https://affnet-dashboard.preview.emergentagent.com/api/admin/login', json=admin_data)
        admin_token = admin_response.json()['token']
        
        # Get firstuser details
        headers = {'Authorization': f'Bearer {admin_token}', 'Content-Type': 'application/json'}
        members_response = requests.get('https://affnet-dashboard.preview.emergentagent.com/api/admin/members?limit=50', headers=headers)
