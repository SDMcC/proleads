...
def test_milestone_integration():
    ...
    response = requests.post('https://marketer-auth-bridge.preview.emergentagent.com/api/admin/login', json=admin_data)
    admin_token = response.json()['token']

    # Get firstuser details
    headers = {'Authorization': f'Bearer {admin_token}', 'Content-Type': 'application/json'}
    response = requests.get('https://marketer-auth-bridge.preview.emergentagent.com/api/admin/members?limit=50', headers=headers)
