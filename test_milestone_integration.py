...
def test_milestone_integration():
    ...
    response = requests.post('https://proleads-hub.preview.emergentagent.com/api/admin/login', json=admin_data)
    admin_token = response.json()['token']

    # Get firstuser details
    headers = {'Authorization': f'Bearer {admin_token}', 'Content-Type': 'application/json'}
    response = requests.get('https://proleads-hub.preview.emergentagent.com/api/admin/members?limit=50', headers=headers)
