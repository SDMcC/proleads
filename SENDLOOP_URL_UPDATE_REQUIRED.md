...
```bash
PROLEADS_API_URL=https://marketer-auth-bridge.preview.emergentagent.com  # ✅ Correct URL
...
3. **Sendloop tries to verify token** with Proleads API ❌ (This fails)
   ...
   - But should call: `https://marketer-auth-bridge.preview.emergentagent.com/api/sso/verify`
...
axios.post(
  'https://marketer-auth-bridge.preview.emergentagent.com/api/sso/verify',
  { sso_token: 'test' },
  {
    headers: {
...
| Endpoint | Full URL |
|----------|----------|
| SSO Initiate | `https://marketer-auth-bridge.preview.emergentagent.com/api/sso/initiate` |
| SSO Verify | `https://marketer-auth-bridge.preview.emergentagent.com/api/sso/verify` |
| SSO User Info | `https://marketer-auth-bridge.preview.emergentagent.com/api/sso/user-info` |
| CSV Export | `https://marketer-auth-bridge.preview.emergentagent.com/api/integrations/csv-export` |
| CSV List | `https://marketer-auth-bridge.preview.emergentagent.com/api/integrations/csv-files` |

### Sendloop Redirect URLs (Proleads uses these)

| Purpose | Full URL |
|---------|----------|
| Dashboard Login | `https://marketer-auth-bridge.preview.emergentagent.com/dashboard` |
| CSV Import | `https://marketer-auth-bridge.preview.emergentagent.com/import` |
...
**Solution:** Update `PROLEADS_API_URL` to `https://marketer-auth-bridge.preview.emergentagent.com`  
