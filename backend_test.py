...
class Web3MembershipTester:
    ...
    def test_notification_history_endpoints(self):
        """Test the new notification history endpoints with pagination"""
        print("\n🔔 Testing Notification History Endpoints")
        
        # First, create a test user and login
        test_user = self.create_test_user_for_notifications()
        if not test_user:
            print("❌ Failed to create test user for notification history testing")
            return False
        ...
        # Test 1: GET /api/users/notifications with default pagination
        print("\n1️⃣ Testing GET /api/users/notifications with default pagination...")
        success, response = self.run_test("Get User Notifications (Default Pagination)", "GET", "users/notifications", 200, headers=headers)
        
        if not success:
            print("❌ Failed to get user notifications with default pagination")
            return False
        
        # Verify response structure
        required_keys = ['notifications', 'total', 'page', 'limit', 'total_pages']
        missing_keys = [key for key in required_keys if key not in response]
        ...
        print("✅ Response contains all required pagination fields")
        
        # Verify default values
        if response.get('page') != 1:
            print(f"❌ Default page should be 1, got {response.get('page')}")
            return False
        
        if response.get('limit') != 10:
            print(f"❌ Default limit should be 10, got {response.get('limit')}")
            return False
        
        print("✅ Default pagination values are correct (page=1, limit=10)")
        
        # Test 2: GET /api/users/notifications with custom pagination
        print("\n2️⃣ Testing GET /api/users/notifications with custom pagination...")
        success, response = self.run_test("Get User Notifications (Custom Pagination)", "GET", "users/notifications?page=1&limit=5", 200, headers=headers)
        
        if not success:
            print("❌ Failed to get user notifications with custom pagination")
            return False
        
        # Verify custom pagination values
        if response.get('page') != 1:
            print(f"❌ Custom page should be 1, got {response.get('page')}")
            return False
        
        if response.get('limit') != 5:
            print(f"❌ Custom limit should be 5, got {response.get('limit')}")
            return False
        
        print("✅ Custom pagination values are correct (page=1, limit=5)")
        
        # Test 3: Verify response structure for empty notifications (expected for new users)
        print("\n3️⃣ Testing response structure for empty notifications...")
        notifications = response.get('notifications', [])
        total = response.get('total', 0)
        total_pages = response.get('total_pages', 0)
        
        # For new users, notifications should be empty
        if len(notifications) == 0 and total == 0 and total_pages == 0:
            print("✅ Empty notifications response is correct for new users")
        else:
            print(f"⚠️ Found {len(notifications)} notifications for new user (total: {total})")
            # This is not necessarily an error - the user might have notifications from referral creation
        
        # Test 4: Verify notification structure if any exist
        if notifications:
            print("\n4️⃣ Testing notification object structure...")
            notification = notifications[0]
            required_notification_keys = ['notification_id', 'user_address', 'type', 'title', 'message', 'created_at', 'read_status']
            missing_notification_keys = [key for key in required_notification_keys if key not in notification]
            ...
            print("4️⃣ No notifications found - skipping notification structure test")
        
        # Test 5: Test unauthorized access
        print("\n5️⃣ Testing unauthorized access...")
        unauth_headers = {'Content-Type': 'application/json'}
        success, response = self.run_test("Get User Notifications (Unauthorized)", "GET", "users/notifications", 401, headers=unauth_headers)
        ...
        # Test 6: Test with different page numbers
        print("\n6️⃣ Testing pagination with different page numbers...")
        success, response = self.run_test("Get User Notifications (Page 2)", "GET", "users/notifications?page=2&limit=10", 200, headers=headers)
        
        if not success:
            print("❌ Failed to get notifications for page 2")
            return False
        
        if response.get('page') != 2:
            print(f"❌ Page 2 should return page=2, got {response.get('page')}")
            return False
        
        print("✅ Page 2 pagination works correctly")
        
        print("\n✅ All Notification History Endpoint Tests Passed")
        return True
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "csv_upload":
        ...
        sys.exit(0 if success else 1)
    elif len(sys.argv) > 1 and sys.argv[1] == "notification_history":
        ...
        success = tester.test_notification_history_endpoints()
    ...
    elif len(sys.argv) > 1 and sys.argv[1] == "lead_enhancements":
        ...
        success = tester.test_lead_distribution_enhancements()
